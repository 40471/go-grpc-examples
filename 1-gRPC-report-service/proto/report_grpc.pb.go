...
const (
	...
	ReportService_StreamReportStatus_FullMethodName = "/report.ReportService/StreamReportStatus"
)
...
type ReportServiceClient interface {
	...
	StreamReportStatus(ctx context.Context, in *StatusRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StatusResponse], error)
}
...
func (c *reportServiceClient) StreamReportStatus(ctx context.Context, in *StatusRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[StatusResponse], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &ReportService_ServiceDesc.Streams[0], ReportService_StreamReportStatus_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
...
type ReportService_StreamReportStatusClient = grpc.ServerStreamingClient[StatusResponse]
...
type ReportServiceServer interface {
	...
	StreamReportStatus(*StatusRequest, grpc.ServerStreamingServer[StatusResponse]) error
...
func (UnimplementedReportServiceServer) StreamReportStatus(*StatusRequest, grpc.ServerStreamingServer[StatusResponse]) error {
	return status.Errorf(codes.Unimplemented, "method StreamReportStatus not implemented")
}
...
func _ReportService_StreamReportStatus_Handler(srv interface{}, stream grpc.ServerStream) error {
	m := new(StatusRequest)
	if err := stream.RecvMsg(m); err != nil {
		return err
	}
	return srv.(ReportServiceServer).StreamReportStatus(m, &grpc.GenericServerStream[StatusRequest, StatusResponse]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type ReportService_StreamReportStatusServer = grpc.ServerStreamingServer[StatusResponse]
...
var ReportService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		{
			StreamName:    "StreamReportStatus",
			Handler:       _ReportService_StreamReportStatus_Handler,
			ServerStreams: true,
		},
	},
